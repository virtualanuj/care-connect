...
async def _data_error(_: Request, exc: Exception) -> JSONResponse:
    # A value the schema allowed but the database cannot store (e.g. integer overflow). Never echo
    # the statement or parameters: they can contain patient data.
    logger.warning("Database rejected a value (%s)", type(exc).__name__)
    return _body(400, ErrorCode.VALIDATION_ERROR, "A submitted value is out of the allowed range")
